...
function WebSocketConnection() {
	//connection = new WebSocket('ws://192.168.1.139:8080/user');
...
class GameScene extends Phaser.Scene {
	...
	create() {
		...
		this.physics.add.overlap(player, fireballs, function(player, fireball1) {
			...
			if(vidas1 > 0){
			player1HP[vidas1 - 1].setVisible(false); vidas1--, dmgSound.play(), fireball1.destroy()
			}

		});
		this.physics.add.overlap(player2, fireballs, function(player2, fireball2) {
			if(vidas2 > 0){
			player2HP[vidas2 - 1].setVisible(false); vidas2--, dmgSound.play(), fireball2.destroy()
			}
	...
	GenerateTraps(contexto, startingX, separation, number) {
		...
		contexto.physics.add.overlap(player, pinchos, function(player, pincho) {
			...
			pincho.destroy();
			if(vidas1 > 0){
			player1HP[vidas1 - 1].setVisible(false); vidas1--;
			}
			dmgSound.play();
		...
		contexto.physics.add.overlap(player2, pinchos, function(player2, pincho) {
			...
			pincho.destroy();
			if(vidas2 > 0){
			player2HP[vidas2 - 1].setVisible(false); vidas2--;
			}
			dmgSound.play();
...
class AjustesUsuarios extends Phaser.Scene {
	...
	create() {
		...
		btnCUsuario.on('pointerdown', () => this.MostrarCambioUsuario(element3, fondoCambioUser, ConfirmarUser, SalirUser, FeedbackAjustes, "", botones));
		ConfirmarUser.on('pointerdown', () => this.ActualizarUsuario(element3));
		SalirUser.on('pointerdown', () => this.EsconderCambioUsuario(element3, fondoCambioUser, ConfirmarUser, SalirUser, FeedbackAjustes, 'changeUserField', botones));

		btnCPassword.on('pointerdown', () => this.MostrarCambioUsuario(element4, fondoCambioPassword, ConfirmarPassword, SalirPassword, FeedbackAjustes, "", botones));
		ConfirmarPassword.on('pointerdown', () => this.ActualizarContrasena(element4));
		SalirPassword.on('pointerdown', () => this.EsconderCambioUsuario(element4, fondoCambioPassword, ConfirmarPassword, SalirPassword, FeedbackAjustes, 'changePasswordField', botones));

		btnBorrar.on('pointerdown', () => this.MostrarCambioUsuario(element2, fondoBorrarCuenta, ConfirmarBorrar, SalirBorrar, FeedbackAjustes, "Esta acci\xf3n es irreversible", botones));
		ConfirmarBorrar.on('pointerdown', () => this.BorrarCuenta());
		SalirBorrar.on('pointerdown', () => this.EsconderBorrado(element2, fondoBorrarCuenta, ConfirmarBorrar, SalirBorrar, FeedbackAjustes, botones));
...
if(data != "error"){
	...
	MostrarCambioUsuario(element, fondo, b1, b2, texto, string, b) {
	...
	EsconderCambioUsuario(element, fondo, b1, b2, texto, string, b) {
	...
	EsconderBorrado(element, fondo, b1, b2, texto, b) {
...
class MainMenu extends Phaser.Scene {
	...
	create() {
		...
		serverText = this.add.text(20, 10, "Server Status", { fontSize: "50px", fill: '#000', stroke: '#000000', strokeThickness: 5 })
		...
		const chatInput = this.add.dom(990, 580, "body").createFromCache("chat");
		...
		function escribirEnChat(chatInput) {
			...
			if(p.value != ""){
			var frase =  nombreActual + ": " + p.value;
...
serverText.setText("Servidor: Conectado");
				serverText.setColor('#00ff00');

		}).catch(error => {

			serverText.setText("Servidor: Desconectado");
...
class LogIn extends Phaser.Scene {
	...
	create() {
		...
		function getUsuario(n, p, e) {

			const datos = { nombre: n, password: p, existe: e };

			$.ajax({

				method: "GET",
				async: false,
				url: '/usuario?nombre=' + datos.nombre + '&password=' + datos.password + '&existe=' + datos.existe,
				processData: false,
				contentType: "application/json"
			}).done(function(data) {
				idGET = data.id;
